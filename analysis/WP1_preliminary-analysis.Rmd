...
pembrolizumab_modifications <- data_01_cycles %>% 
  ...
  mutate(
    ...
    temporary_interruption = if_else(
      pembrolizumab_administered == "No" & !is.na(reason_no_administration) & !str_detect(reason_no_administration, "temporal"), "Yes", "No"),
    delay = if_else(
...
tbl_pembrolizumab_delays <- pembrolizumab_delays %>%
  tbl_summary(
    ...
    label = list(delay ~ "Pembrolizumab dose delay",
                 treatment_delay_reason ~ "Reason for treatment delay"),
...
tbl_pembrolizumab_temporary <- pembrolizumab_temporary_interruptions %>%
  tbl_summary(
    ...
    type = all_dichotomous() ~ "categorical",
    sort = list(everything() ~ "frequency"),
    label = list(temporary_interruption ~ "Pembrolizumab temporary dose interruption",
                 reason_no_administration ~ "Reason for interruption"),
...
tbl_lenvatinib_temporary <- lenvatinib_temporary_interruptions %>%
  tbl_summary(
    ...
    label = list(temporary_interruption ~ "Lenvatinib temporary dose interruption",
                 reason_of_no_dose_other_dose_or_dose_reduction ~ "Reason for interruption"),
...
### **Table 5. Study drugs dose modifications**
...
tbl_stack %>% modify_spanning_header(all_stat_cols() ~ "")
```
...
```{r, message = FALSE, warning = FALSE, echo = FALSE}
saes_g3 <- saes %>% 
  filter(grade == c("Grade 3", "Grade 4", "Grade 5"))
