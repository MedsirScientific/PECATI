---
title: "PECATI | Preliminary data monitoring, cleaning, and analysis. June 2024"
...
project_id <- "PECATI"
cutoff_date <- as.Date("2024-06-20")
```
...
# Step 1: Filter patients who failed screening due to "Non-fulfillment Selection Criteria"
screening_failures <- data_00_screening %>%
  filter(sf_reason == "Non-fulfillment Selection Criteria")

# Step 2: Identify inclusion and exclusion criteria columns
inclusion_criteria_cols <- grep("inclusion_criteria", colnames(screening_failures), value = TRUE)
exclusion_criteria_cols <- grep("exclusion_criteria", colnames(screening_failures), value = TRUE)

# Step 3: Check which criteria were not met for each patient
# For inclusion criteria, "No" indicates failure
# For exclusion criteria, "Yes" indicates failure

# Create a function to summarize the failed criteria
find_failed_criteria <- function(patient_row) {
  failed_inclusion <- inclusion_criteria_cols[which(patient_row[inclusion_criteria_cols] == "No")]
  failed_exclusion <- exclusion_criteria_cols[which(patient_row[exclusion_criteria_cols] == "Yes")]
  
  failed_criteria <- c(failed_inclusion, failed_exclusion)
  return(failed_criteria)
}

# Apply the function to each row in the screening_failures dataset
screening_failures <- screening_failures %>%
  rowwise() %>%
  mutate(failed_criteria = list(find_failed_criteria(cur_data())))

# Ungroup to return to a data frame
screening_failures <- ungroup(screening_failures)
screening_failures <- screening_failures %>%
  select(patient, failed_criteria)

criteria_list <- list(
  inclusion_criteria_2_v2 = "No progression to prior platinum for advanced disease",
  inclusion_criteria_3_v2 = "Myasthenia Gravis",
  inclusion_criteria_5_v2 = "ECOG ≥2",
  inclusion_criteria_7_v2 = "Radiological progression to a prior line",
  exclusion_criteria_3_v2 = "Prior therapy with sunitinib",
  exclusion_criteria_7_v2 = "Proteinuria",
  exclusion_criteria_24_v2 = "Prior radiotherapy within 2 weeks",
  exclusion_criteria_26_v2 = "Investigator decision",
  inclusion_criteria_1_v3 = "No relapsed/recurrent B3-Thymoma or TC, or amenable to curative intent",
  inclusion_criteria_3_v3 = "Myasthenia Gravis",
  inclusion_criteria_9_v3 = "Unable to provide tumor biopsy specimens",
  exclusion_criteria_6_v3 = "Intratumor cavitation, invasion of mediastinal blood vessels or bleeding",
  exclusion_criteria_11_v3 = "Not adequately recovered from prior interventions"
)

### Manual fix ### ------------------------------------------------------------
### Mapping for a patient with specific criteria to keep given more than one unmet criteria
patient_criteria_mapping <- list(
  "0103-003" = "exclusion_criteria_3_v2"
)

# Function to filter and keep specified criteria for certain patients
filter_criteria <- function(patient_id, criteria, patient_criteria_mapping) {
  if (patient_id %in% names(patient_criteria_mapping)) {
    return(patient_criteria_mapping[[patient_id]])
  }
  return(criteria)
}

# Function to substitute criteria codes with explanations
substitute_criteria <- function(criteria_codes, criteria_list) {
  criteria_explanations <- map(criteria_codes, ~ criteria_list[[.x]])
  return(criteria_explanations)
}

# Apply the function to filter criteria and substitute explanations
screening_failures <- screening_failures %>%
  rowwise() %>%
  mutate(failed_criteria = list(filter_criteria(patient, failed_criteria, patient_criteria_mapping))) %>%
  ungroup() %>%
  mutate(
    failed_criteria_explanations = map(
      failed_criteria, substitute_criteria, criteria_list
      ),
    failed_criteria_explanations = map_chr(
      failed_criteria_explanations, ~ paste(unlist(.), collapse = "; ")
      )
    ) %>% 
  select(
    patient, failed_criteria_explanations
  )

data_00_screening <- merge(
  data_00_screening,
  screening_failures,
  by = "patient",
  all = TRUE
  ...
  select(
    patient, screening_date, eligible, sf_date, sf_reason, failed_criteria_explanations, eligibility_date
    )
...
### Query ### -----------------------------------------------------------------  ### Query ###
### What happened to the eligibility date of patient 0301-014? ###-------------
### ---------------------------------------------------------------------------  ### Query ###
```
...
listing1_formatted_names <- c(
  ...
  "Main unmet inclusion or exclusion criteria",
...
data_00_screening %>%
  tbl_summary(
    include = c(eligible, sf_reason, failed_criteria_explanations),
    missing = "no",
    label = list(
      eligible ~ "Included",
      sf_reason ~ "Excluded",
      failed_criteria_explanations ~ "Main unmet criteria"
    ),
...
data_06_eos <- read_excel(
  ...
  mutate(
    ...
    exitus_reason = if_else(
      patient == "0103-001", "Disease progression", exitus_reason
    ),
    exitus_reason = if_else(
      patient == "0201-001", "Disease progression", exitus_reason
    ),
    exitus_reason = if_else(
...
data_temp_eos <- data_temp_eos %>%
  mutate(
    clinical_pd = if_else(
      eot_reason == "Clinical progression", "Yes", clinical_pd
    ),
    clinical_pd_date = if_else(
      clinical_pd == "Yes" & is.na(clinical_pd_date), discontinuation_date, clinical_pd_date
    ),
    exitus = if_else(
    ...
    progression = if_else(
      is.na(progression) & clinical_pd == "Yes", discontinuation_date, progression
    ),
    progression = if_else(
      is.na(progression) & !is.na(exitus), exitus, progression
    )
  )
...
### Under development ### -----------------------------------------------------
data_07_swimmer_long$eos_this_day <- ifelse(
  !is.na(data_07_swimmer_long$eos) & data_07_swimmer_long$eos_date == data_07_swimmer_long$study_day, data_07_swimmer_long$month, NA
)
### ---------------------------------------------------------------------------
...
swimmer <- data_07_swimmer_long %>%
  ...
  geom_line(aes(x = month), linewidth = 2.5, color = "#E1E1E4") +
  geom_point(aes(x = eos_this_day),
    size = 5.5,
    stroke = 1.5,
    shape = 73,
    color = "#E1E1E4",
  ...
  # geom_line(data = subset(data_07_swimmer_long, eos == "Yes"), aes(x = month), linewidth = 2.5, color = "#2a2a2a", alpha = 0.25, arrow = arrow(type = "closed")) +
...
Blue filled circles represent the date of informed consent. Blue empty circles represent the date of study treatment start. Yellow filled circles represent the date of disease progression. Black filled circles represent the date of exitus. The bullseye represents an EoS.
...
write_xlsx(
  data_12_or_recist,
  here(paste0("output/", project_id, "_12_or_recist_", cutoff_date, ".xlsx"))
  )
...
-   Patient 0102-003 discontinued study treatment after 1 cycle without any post-baseline tumor assessment due to clinical progression.
...
-   Patient 0103-002 is stable after 34 cycles of study treatment (last registered Lenvatinib dose: 2024-07-10; last registered Pembrolizumab dose: 2024-08-01).
...
-   Patient 0105-001 discontinued study treatment after completing the maximum number of 35 cycles allowed (last registered Lenvatinib dose: 2024-05-28; last registered Pembrolizumab dose: 2024-05-08).
...
-   Patient 0201-001 discontinued study treatment after 17 cycles due to radiological progression (unequivocal progression of non-target lesions and new hepatic lesion).

-   Patient 0201-002 discontinued study treatment after 11 cycles due to loss to follow-up (administrative reasons).

-   Patient 0201-003 discontinued study treatment after 22 cycles due to unacceptable toxicity (G2 Diarrhoea).

-   Patient 0201-004 is stable after 21 cycles of study treatment (last registered Lenvatinib dose: 2024-07-30; last registered Pembrolizumab dose: 2024-07-31).
...
-   Patient 0202-004 is responding after 26 cycles of study treatment (last registered Lenvatinib dose: 2023-06-24; last registered Pembrolizumab dose: 2024-06-25).
...
-   Patient 0202-007 discontinued the study treatment after 11 cycles due to administrative reasons.

-   Patient 0202-008 is stable after 10 cycles of study treatment (last registered Lenvatinib dose: 2024-06-30; last registered Pembrolizumab dose: 2024-06-10).
...
-   Patient 0203-001 is stable after 23 cycles of study treatment (last registered Lenvatinib dose: 2024-02-22; last registered Pembrolizumab dose: 2023-08-07).

-   Patient 0203-002 is responding after 14 cycles of study treatment (last registered Lenvatinib dose: 2024-07-03; last registered Pembrolizumab dose: 2023-06-11).

-   Patient 0203-003 discontinued the study treatment after 2 cycle due to radiological progression (unequivocal progression of non-target lesions).

-   Patient 0203-004 is responding after 9 cycles of study treatment (last registered Lenvatinib dose: 2024-06-20; last registered Pembrolizumab dose: 2023-06-21).
...
-   Patient 0204-001 is stable after 20 cycles of study treatment (last registered Lenvatinib dose: 2024-07-29; last registered Pembrolizumab dose: 2024-07-30).

-   Patient 0204-003 discontinued study treatment after 7 cycles due to toxicity (G3 Cholestasis and G3 Cytolysis related to all study drugs).
...
-   Patient 0205-001 discontinued study treatment after 14 cycles due to investigator decision (last registered Lenvatinib dose: 2024-01-23; last registered Pembrolizumab dose: 2024-02-14).

-   Patient 0205-002 discontinued study treatment after 6 cycles due to unacceptable toxicity (G2 Acute heart failure).
...
-   Patient 0206-001 discontinued study treatment after 14 cycles due to radiological progression (new lymph node lesions).

-   Patient 0206-002 discontinued study treatment after 6 cycles due to radiological progression (new lesions in the spleen). Please note that the **EoT page has not been created** yet.
...
-   Patient 0301-003 is responding after 31 cycles of study treatment (last registered Lenvatinib dose: 2023-08-01; last registered Pembrolizumab dose: 2024-08-02).

-   Patient 0301-004 is stable after 27 cycles of study treatment (last registered Lenvatinib dose: 2024-07-24; last registered Pembrolizumab dose: 2024-07-25).

-   Patient 0301-005 discontinued the study treatment after 10 cycles due to clinical progression.

-   Patient 0301-006 is stable after 24 cycles of study treatment (last registered Lenvatinib dose: 2024-08-07; last registered Pembrolizumab dose: 2024-08-04).

-   Patient 0301-007 is stable after 22 cycles of study treatment (last registered Lenvatinib dose: 2024-07-31; last registered Pembrolizumab dose: 2024-07-11).

-   Patient 0301-008 discontinued the study treatment after 2 cycles due to radiological progression (new lesions in the lung).

-   Patient 0301-009 is stable after 19 cycles of study treatment (last registered Lenvatinib dose: 2024-07-31; last registered Pembrolizumab dose: 2024-08-01).

-   Patient 0301-010 is stable after 19 cycles of study treatment (last registered Lenvatinib dose: 2024-08-01; last registered Pembrolizumab dose: 2024-08-02).

-   Patient 0301-011 discontinued the study treatment after 14 due to radiological progression (new hepatic lesions).
...
-   Patient 0301-013 is responding after 13 cycles of study treatment (last registered Lenvatinib dose: 2024-08-01; last registered Pembrolizumab dose: 2024-08-02).

-   Patient 0301-014 discontinued the study treatment after 3 cycles due to investigator decision.

## Last Lenvatinib Dose

```{r, message = FALSE, warning = FALSE, echo = FALSE}
last_lenvatinib <- data_01_cycles %>%
  select(patient, event_num, visit_date, has_the_patient_taken_any_lenvatinib_dose_during_the_cycle) %>%
  filter(has_the_patient_taken_any_lenvatinib_dose_during_the_cycle == "Yes") %>%
  group_by(patient) %>%
...
patients_not_in_last_lenvatinib <- data_02_itt %>%
  anti_join(last_lenvatinib, by = "patient") %>%
  select(patient)

# last_lenvatinib <- last_lenvatinib %>%
#   bind_rows(
#     data_02_itt %>%
#       anti_join(last_lenvatinib, by = "patient") %>%
#       select(patient) %>%
...
#         event_num = NA,
#         visit_date = NA,
#         has_the_patient_taken_any_lenvatinib_dose_during_the_cycle = NA
#       )
#   )

last_lenvatinib <- last_lenvatinib %>%
  rename(
    last_cycle_visit_date = visit_date
  )
...
lenvatinib <- merge(
  ...
  select(-has_the_patient_taken_any_lenvatinib_dose_during_the_cycle) %>%
  # Create a new variable with the number of weeks between last_cycle_visit_date dose and c1d1_date
  mutate(
    last_lenvatinib = if_else(
      is.na(last_cycle_visit_date), NA, as.numeric(last_cycle_visit_date - c1d1_date, units = "weeks")
    ),
    lenvatinib_8w = if_else(
      last_lenvatinib >= 8, "Yes", "No"
    )
  )
```

#### **Table 4. Number of weeks receiving Lenvatinib:**
...
lenvatinib %>%
  tbl_summary(
    missing = "always",
    percent = "column",
    type = list(lenvatinib_8w ~ "categorical"),
    sort = list(everything() ~ "frequency"),
    label = list(lenvatinib_8w ~ "At least 8 weeks of Lenvatinib"),
    include = c(lenvatinib_8w),
...
# Extent of Exposure to the Study Treatment

## Dose delays, reductions, and interruptions

```{r, message = FALSE, warning = FALSE, echo = FALSE}
### Pembrolizumb ### ----------------------------------------------------------
pembrolizumab_modifications <- data_01_cycles %>%
  mutate(
    visit_date = dmy(visit_date)
  ) %>%
  select(
    patient, event_num, pembrolizumab_administered, reason_no_administration, specify_ae_no_dose, date_of_administration,
    treatment_delay_reason, pembrolizumab_administered_dose, other_dose_mg
  ) %>%
  mutate(
    date_of_administration = dmy(date_of_administration),
    temporary_interruption = if_else(
      pembrolizumab_administered == "No" & !is.na(reason_no_administration) & !str_detect(reason_no_administration, "permanent"), "Yes", "No"
    ),
    delay = if_else(
      pembrolizumab_administered == "Yes" & !is.na(treatment_delay_reason), "Yes", "No"
    ),
    reduction = if_else(
      !is.na(other_dose_mg), "Yes", "No"
    ),
    permanent_discontinuation = if_else(
      pembrolizumab_administered == "No" & str_detect(reason_no_administration, "permanent"), "Yes", "No"
    )
  ) %>%
  select(
    patient, event_num, pembrolizumab_administered, date_of_administration,
    delay, treatment_delay_reason,
    reduction, pembrolizumab_administered_dose, other_dose_mg,
    temporary_interruption, reason_no_administration,
    permanent_discontinuation
  )

### Lenvatinib ### ------------------------------------------------------------
lenvatinib_modifications_s1 <- data_01_cycles %>%
  mutate(
    visit_date = dmy(visit_date)
  ) %>%
  select(
    patient, event_num, has_the_patient_taken_any_lenvatinib_dose_during_the_cycle, number_capsules_taken_10_mg, number_capsules_taken_4_mg
  ) %>%
  mutate(
    lenvatinib_dose_mg = (number_capsules_taken_10_mg * 10) + (number_capsules_taken_4_mg * 4)
  )

lenvatinib_modifications_s2 <- read_excel(
  here(paste0("data/", cutoff_date, "/", project_id, "_Cycles_", cutoff_date_formatted, ".xlsx")),
  sheet = "LENVATINIB",
  skip = 1
) %>%
  ...
  mutate(
    date = dmy(date),
    lenvatinib_dose_mg = case_when(
      dose == "10 mg (1 capsule 10 mg)" ~ 10,
      dose == "12 mg (3 capsules 4 mg)" ~ 12,
      dose == "16 mg (4 capsules 4 mg )" ~ 16,
      dose == "20 mg (2 capsules 10 mg)" ~ 20,
      dose == "No dose" ~ 0
    )
  ) %>%
  group_by(patient, event_num) %>%
  mutate(
    lenvatinib_daily_dose_mg = sum(lenvatinib_dose_mg)
  ) %>%
  filter(
    day == "#1"
  ) %>%
  select(
    patient, event_num, date, lenvatinib_daily_dose_mg,
    reason_of_no_dose_other_dose_or_dose_reduction, specify_ae_interruption
  )

lenvatinib_modifications <- lenvatinib_modifications_s1 %>%
  left_join(
    lenvatinib_modifications_s2,
    by = c("patient", "event_num")
  ) %>%
  rename(
    lenvatinib_administered = has_the_patient_taken_any_lenvatinib_dose_during_the_cycle,
  ) %>%
  select(
    patient, event_num, date, lenvatinib_administered, lenvatinib_daily_dose_mg,
    reason_of_no_dose_other_dose_or_dose_reduction, specify_ae_interruption
  )
...
pembrolizumab_delay <- pembrolizumab_modifications %>%
  group_by(patient) %>%
  filter(
    delay == "Yes"
  ) %>%
  filter(
    event_num == min(event_num)
  ) %>%
  ungroup() %>%
  select(-event_num)

## Patients without any treatment delay
pembrolizumab_no_delay <- pembrolizumab_modifications %>%
  group_by(patient) %>%
  filter(
    delay == "No"
  ) %>%
...
pembrolizumab_no_delay_clean <- pembrolizumab_no_delay %>%
  anti_join(pembrolizumab_delay, by = "patient") %>%
  select(patient, everything())

pembrolizumab_delays <- bind_rows(pembrolizumab_delay, pembrolizumab_no_delay_clean)

pembrolizumab_delays <- pembrolizumab_delays %>%
  mutate(
    treatment_delay_reason = if_else(
      treatment_delay_reason == "Adverse Event/toxicity (please, record it in the AEs form)", "Adverse Event/toxicity", treatment_delay_reason
    )
  )
```
...
```{r, message = FALSE, warning = FALSE, echo = FALSE}
# temp <- read_excel( # lenvatinib_modifications_s2
#   here(paste0("data/", cutoff_date, "/", project_id, "_Cycles_", cutoff_date_formatted, ".xlsx")),
#   sheet = "LENVATINIB",
#   skip = 1
# ) %>%
...
# temp_min <- temp %>%
#   group_by(patient) %>% 
#   filter(lenvatinib_dose_mg != 0) %>% 
#   filter(lenvatinib_dose_mg == min(lenvatinib_dose_mg) & lenvatinib_dose_mg != 0) %>%
#   filter(row_number() == 1) %>% 
#   # Summarize column lenvatinib_dose_mg
#   summarize(
#     lenvatinib_dose_mg = sum(lenvatinib_dose_mg)
#   )
# 
# temp_table <- temp_min %>%
#   tbl_summary(
...
#     sort = list(everything() ~ "frequency"),
#     label = list(
#       lenvatinib_dose_mg ~ "Min Lenvatinib dose"
#     ),
#     include = c(lenvatinib_dose_mg)
#   ) %>%
#   modify_header(label = " ") %>%
#   bold_labels()
# 
...
<!-- #### **Table X.1. Pembrolizumab dose delay:** -->

```{r, message = FALSE, warning = FALSE, echo = FALSE}
tbl_pembrolizumab_delays <- pembrolizumab_delays %>%
  tbl_summary(
    missing = "no",
    percent = "column",
    type = all_dichotomous() ~ "categorical",
    sort = list(everything() ~ "frequency"),
    label = list(
      delay ~ "Pembrolizumab dose delay",
      treatment_delay_reason ~ "Reason for dose delay"
    ),
    include = c(delay, treatment_delay_reason)
  ) %>%
  modify_header(label = " ") %>%
  bold_labels()
```
...
### Pembrolizumb ### ----------------------------------------------------------
# Treatment reduction
## Patients with treatment reduction
pembrolizumab_reduction <- pembrolizumab_modifications %>%
  group_by(patient) %>%
  filter(
    reduction == "Yes"
  ) %>%
...
## Patients without any treatment reduction
pembrolizumab_no_reduction <- pembrolizumab_modifications %>%
  group_by(patient) %>%
  filter(
    reduction == "No"
  ) %>%
...
pembrolizumab_no_reduction_clean <- pembrolizumab_no_reduction %>%
  anti_join(pembrolizumab_reduction, by = "patient") %>%
  select(patient, everything())

pembrolizumab_reductions <- bind_rows(pembrolizumab_reduction, pembrolizumab_no_reduction_clean)

### Lenvatinib ### ------------------------------------------------------------
# Treatment reduction
## Patients with treatment reduction
lenvatinib_reductions <- lenvatinib_modifications %>%
  group_by(patient) %>%
  summarise(
    mean_daily_dose = mean(lenvatinib_daily_dose_mg, na.rm = TRUE),
    min_daily_dose = min(lenvatinib_daily_dose_mg, na.rm = TRUE),
    max_daily_dose = max(lenvatinib_daily_dose_mg, na.rm = TRUE)
  ) %>%
  ungroup()

### The following information was a petition from the PI ### ------------------
# Print the median of mean_daily_dose
# mean(lenvatinib_reductions$mean_daily_dose, na.rm = TRUE)
# min(lenvatinib_reductions$min_daily_dose, na.rm = TRUE)
# max(lenvatinib_reductions$max_daily_dose, na.rm = TRUE)

### Line and dot plot of lenvatinib daily dose across time ### ----------------
# lenvatinib_modifications_s2 %>%
#   ggplot(aes(x = event_num, y = lenvatinib_daily_dose_mg, group = patient)) +
#   geom_line() +
#   geom_point() +
#   theme_minimal() +
#   labs(
#     title = "Lenvatinib daily dose (mg) across time",
#     x = "Cycle number",
#     y = "Lenvatinib daily dose (mg)"
#   )
```

#### **Figure 3. Total Lenvatinib dose (mg) per cycle across time**

```{r, line-dot-plot-lenvatinib, fig.height = 15, message = FALSE, warning = FALSE, echo = FALSE}
lenvatinib_daily_dose <- lenvatinib_modifications_s2 %>%
  ggplot(aes(x = event_num, y = lenvatinib_daily_dose_mg, group = patient)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(
    title = "Total Lenvatinib dose (mg) per cycle across time",
    x = "Cycle number",
    y = "Lenvatinib daily dose (mg)"
  ) +
  facet_wrap(~patient) # +
...
### Save plot ---------------------------------------------------------------
ggsave(
  paste("../output/", project_id, "_lenvatinib_dose", as.Date(Sys.Date()), ".png", sep = ""),
  lenvatinib_daily_dose,
  width = 50,
  height = 50,
  units = "cm",
  dpi = 300
...
<!-- #### **Table X.2. Pembrolizumab dose reduction:** -->

```{r, message = FALSE, warning = FALSE, echo = FALSE}
tbl_pembrolizumab_reductions <- pembrolizumab_reductions %>%
  tbl_summary(
    missing = "no",
    percent = "column",
    type = all_dichotomous() ~ "categorical",
    sort = list(everything() ~ "frequency"),
    label = list(
      reduction ~ "Pembrolizumab dose reduction",
      pembrolizumab_administered_dose ~ "Dose administered"
    ),
    include = c(reduction, pembrolizumab_administered_dose)
  ) %>%
  modify_header(label = " ") %>%
  bold_labels()
```
...
```{r, message = FALSE, warning = FALSE, echo = FALSE}
### Pembrolizumab ### ---------------------------------------------------------
# Treatment temporary interruption
## Patients with treatment temporary interruption
pembrolizumab_temporary <- pembrolizumab_modifications %>%
  group_by(patient) %>%
  filter(
    temporary_interruption == "Yes"
  ) %>%
  filter(
    event_num == min(event_num)
  ) %>%
...
## Patients without any treatment reduction
pembrolizumab_no_temporary <- pembrolizumab_modifications %>%
  group_by(patient) %>%
  filter(
    temporary_interruption == "No"
  ) %>%
  filter(
    event_num == min(event_num)
  ) %>%
...
pembrolizumab_no_temporary_clean <- pembrolizumab_no_temporary %>%
  anti_join(pembrolizumab_temporary, by = "patient") %>%
  select(patient, everything())

pembrolizumab_temporary_interruptions <- bind_rows(pembrolizumab_temporary, pembrolizumab_no_temporary_clean)

pembrolizumab_temporary_interruptions <- pembrolizumab_temporary_interruptions %>%
  mutate(
    reason_no_administration = if_else(
      reason_no_administration == "Dose withhold due to toxicity or other medical reason (record in AEs form)", "Adverse Event/toxicity", reason_no_administration
    ),
    reason_no_administration = if_else(
      reason_no_administration == "Non-Medical Reason", "Non-medical reasons", reason_no_administration
    )
  )
...
# Treatment temporary interruption
## Patients with treatment temporary interruption
lenvatinib_temporary <- lenvatinib_modifications %>%
  group_by(patient) %>%
  filter(
    str_detect(lenvatinib_administered, "temporal")
  ) %>%
  filter(
    event_num == min(event_num)
  ) %>%
...
## Patients without any treatment reduction
lenvatinib_no_temporary <- lenvatinib_modifications %>%
  group_by(patient) %>%
  filter(
    !str_detect(lenvatinib_administered, "temporal")
  ) %>%
...
lenvatinib_no_temporary_clean <- lenvatinib_no_temporary %>%
  anti_join(lenvatinib_temporary, by = "patient") %>%
  select(patient, everything())

lenvatinib_temporary_interruptions <- bind_rows(lenvatinib_temporary, lenvatinib_no_temporary_clean)

lenvatinib_temporary_interruptions <- lenvatinib_temporary_interruptions %>%
  mutate(
    temporary_interruption = if_else(
      str_detect(lenvatinib_administered, "temporal"), "Yes", "No"
    ),
    reason_of_no_dose_other_dose_or_dose_reduction = if_else(
      reason_of_no_dose_other_dose_or_dose_reduction == "Toxicity (record in AEs form)", "Adverse Event/toxicity", reason_of_no_dose_other_dose_or_dose_reduction
    ),
    reason_of_no_dose_other_dose_or_dose_reduction = if_else(
      temporary_interruption == "Yes" & reason_of_no_dose_other_dose_or_dose_reduction == "NA", "Unknown", reason_of_no_dose_other_dose_or_dose_reduction
    )
  )
```

<!-- #### **Table X.3. Pembrolizumab temporary dose interruption:** -->

```{r, message = FALSE, warning = FALSE, echo = FALSE}
tbl_pembrolizumab_temporary <- pembrolizumab_temporary_interruptions %>%
  tbl_summary(
    missing = "no",
    percent = "column",
    type = all_dichotomous() ~ "categorical",
    sort = list(everything() ~ "frequency"),
    label = list(
      temporary_interruption ~ "Pembrolizumab dose temporary interruption",
      reason_no_administration ~ "Reason for temporary interruption"
    ),
    include = c(temporary_interruption, reason_no_administration)
  ) %>%
  modify_header(label = " ") %>%
  bold_labels()
```

<!-- #### **Table X.3. Lenvatinib temporary dose interruption:** -->

```{r, message = FALSE, warning = FALSE, echo = FALSE}
tbl_lenvatinib_temporary <- lenvatinib_temporary_interruptions %>%
  tbl_summary(
    missing = "no",
    percent = "column",
    type = all_dichotomous() ~ "categorical",
    sort = list(everything() ~ "frequency"),
    label = list(
      temporary_interruption ~ "Lenvatinib temporary dose interruption",
      reason_of_no_dose_other_dose_or_dose_reduction ~ "Reason for temporary interruption"
    ),
    include = c(temporary_interruption, reason_of_no_dose_other_dose_or_dose_reduction)
  ) %>%
  modify_header(label = " ") %>%
  bold_labels()
```
...
## Patients with treatment permanent discontinuation
pembrolizumab_permanent <- pembrolizumab_modifications %>%
  group_by(patient) %>%
  filter(
    permanent_discontinuation == "Yes"
  ) %>%
  filter(
    event_num == min(event_num)
  ) %>%
...
## Patients without any treatment reduction
pembrolizumab_no_permanent <- pembrolizumab_modifications %>%
  group_by(patient) %>%
  filter(
    permanent_discontinuation == "No"
  ) %>%
  filter(
    event_num == min(event_num)
  ) %>%
...
pembrolizumab_no_permanent_clean <- pembrolizumab_no_permanent %>%
  anti_join(pembrolizumab_permanent, by = "patient") %>%
  select(patient, everything())

pembrolizumab_permanent_discontinuations <- bind_rows(pembrolizumab_permanent, pembrolizumab_no_permanent_clean)

### Lenvatinib ### ------------------------------------------------------------
# Treatment permanent discontinuation
## Patients with treatment permanent discontinuation
lenvatinib_permanent <- lenvatinib_modifications %>%
  group_by(patient) %>%
  filter(
    str_detect(lenvatinib_administered, "permanent")
  ) %>%
  filter(
    event_num == min(event_num)
  ) %>%
...
## Patients without any treatment reduction
lenvatinib_no_permanent <- lenvatinib_modifications %>%
  group_by(patient) %>%
  filter(
    !str_detect(lenvatinib_administered, "permanent")
  ) %>%
...
lenvatinib_no_permanent_clean <- lenvatinib_no_permanent %>%
  anti_join(lenvatinib_permanent, by = "patient") %>%
  select(patient, everything())

lenvatinib_permanent_interruptions <- bind_rows(lenvatinib_permanent, lenvatinib_no_permanent_clean)

lenvatinib_permanent_interruptions <- lenvatinib_permanent_interruptions %>%
  mutate(
    permanent_interruption = if_else(
      str_detect(lenvatinib_administered, "permanent"), "Yes", "No"
    ),
    lenvatinib_administered = if_else(
      lenvatinib_administered == "No, temporal interruption", "Yes", lenvatinib_administered
    )
  )
```

<!-- #### **Table X.4. Pembrolizumab permanent discontinuation:** -->

```{r, message = FALSE, warning = FALSE, echo = FALSE}
tbl_pembrolizumab_permanent <- pembrolizumab_permanent_discontinuations %>%
  tbl_summary(
    missing = "no",
    percent = "column",
    type = all_dichotomous() ~ "categorical",
    sort = list(everything() ~ "frequency"),
    label = list(permanent_discontinuation ~ "Pembrolizumab permanent discontinuation"),
    include = c(permanent_discontinuation)
  ) %>%
  modify_header(label = " ") %>%
  bold_labels()
```

<!-- #### **Table X.3. Lenvatinib permanent discontinuation:** -->

```{r, message = FALSE, warning = FALSE, echo = FALSE}
tbl_lenvatinib_permanent <- lenvatinib_permanent_interruptions %>%
  tbl_summary(
    missing = "no",
    percent = "column",
    type = all_dichotomous() ~ "categorical",
    sort = list(everything() ~ "frequency"),
    label = list(permanent_interruption ~ "Lenvatinib permanent interruption"),
    include = c(permanent_interruption)
  ) %>%
  modify_header(label = " ") %>%
  bold_labels()
```

### **Table 5. Study drugs modifications**

```{r, message = FALSE, warning = FALSE, echo = FALSE}
row1 <- tbl_merge(list(tbl_pembrolizumab_delays))
row2 <- tbl_merge(list(tbl_pembrolizumab_reductions))
row3 <- tbl_merge(list(tbl_pembrolizumab_temporary))
row4 <- tbl_merge(list(tbl_pembrolizumab_permanent))
row5 <- tbl_merge(list(tbl_lenvatinib_temporary))
row6 <- tbl_merge(list(tbl_lenvatinib_permanent))
...
#### **Table 7. Progression-free survival (PFS) in the intention-to-treat population:**
...
#### **Figure 4. Progression-free survival (PFS) in the intention-to-treat population**
...
caption <- paste(strwrap(
  "5-month PFS is 90.7% (95% CI 82.4-99.8)", 90
), collapse = "\n")
...
#### **Figure 5. Overall survival (OS) in the intention-to-treat population**
...
os_global <- ggsurvplot(
  ...
  xlab = "Time since start of the study treatment (months)",
...
os_global <- customize_labels(
  ...
  font.x         = c(11, "bold",  "#0a0908"),
  font.y         = c(11, "bold",  "#0a0908"),
  font.xtickslab = c(8, "plain", "#0a0908"),
  font.ytickslab = c(8, "plain", "#0a0908")
)
...
caption1 <- paste(strwrap(
  "Medidan OS has not been reached yet (14.8 - NR). Events: 9/43 (20.9%)", 90
), collapse = "\n")

caption2 <- paste(strwrap(
  "12-month OS is 85.9% (95% CI 69.0-94.0)", 90
), collapse = "\n")
...
#### **Table 11. Summary of PFS events:**
...
#### **Figure 6. Progression-free survival (PFS) in the intention-to-treat population**
...
caption1 <- paste(strwrap(
  "Medidan PFS has not been reached yet (12.9 - NR). Events: 10/43 (23.3%)", 90
), collapse = "\n")

caption2 <- paste(strwrap(
  "12-month PFS is 78.3% (95% CI 58.7-89.4)", 90
), collapse = "\n")
...
data_14_aes <- read_excel(
  ...
  filter(adverse_event != c("Death", "death")) %>%
  rename(
...
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Asthenia", "Fatigue")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Arterial Hypertension", "Hypertension")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Hand Neuropathies", "Neuropathy")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Intermittent Diarrhea", "Diarrhea")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Appetite Loss", "Decreased appetite")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Loss Of Appetite", "Decreased appetite")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Pain On Toe Of Right Foot", "Pain")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Polyneuropathy (Hands)", "Neuropathy")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Shoulder Pain Bilaterally", "Pain")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Worsening Of Abdominal Pain", "Pain")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Systolic Hypertension", "Hypertension")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Diarrhea", "Diarrhoea")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Diahrrea", "Diarrhoea")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Systolic Hypertension", "Hypertension")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Dyspnea", "Dyspnoea")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Exertional Dyspnoea", "Dyspnoea")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Abdominal Pain", "Pain")
data_14_aes$adverse_event <- str_replace_all(data_14_aes$adverse_event, "Abdominal Pains", "Pain")

data_14_aes$grade <- str_remove(data_14_aes$grade, "/.*")

myocarditis <- data_14_aes %>%
  filter(adverse_event == "Myocarditis")
...
# Create a table to show which patients have any adverse event of grade "Not applicable"
...
#### **Table 14. Adverse Events (AEs) by maximum severity in the ITT population:**

```{r, message = FALSE, warning = FALSE, echo = FALSE}
data_14_aes <- data_14_aes %>%
  filter(grade != "Not aplicable")
...
#### **Table 15. Serious Adverse Events (SAEs) by maximum severity in the ITT population:**
...
#### **Table 16. Serious Adverse Events (SAEs) of grade ≥3 in the ITT population:**

```{r, message = FALSE, warning = FALSE, echo = FALSE}
# Select adverse_events of grade 3, 4, and 5
saes_g3 <- saes %>%
  filter(grade %in% c("Grade 3", "Grade 4", "Grade 5"))
...
tbl_saes_g3 %>%
  as_flex_table() %>%
  flextable::save_as_docx(
    path = here(paste0("output/", project_id, "_saes_g3_table_", as.Date(Sys.Date()), ".docx"))
  )

tbl_saes_g3
```

There is `r nrow(myocarditis)` patient with `r myocarditis$adverse_event` of `r tolower(myocarditis$grade)` `r tolower(myocarditis$relationship)`.

#### **Table 17. Adverse Events (AEs) that led to the permanent discontinuation of the study drug in the ITT population:**

```{r, message = FALSE, warning = FALSE, echo = FALSE}
aes_discontinuation <- data_14_aes %>%
  filter(!str_detect(action_taken, "permanent"))

aes_discontinuation_pembro <- aes_discontinuation %>%
  filter(str_detect(action_taken, "Pembrolizumab"))

aes_discontinuation_lenva <- aes_discontinuation %>%
  filter(str_detect(action_taken, "Lenvatinib"))
...
There are `r length(unique(aes_discontinuation$patient))` patients with adverse events that led to the permanent discontinuation of any study drug. Pembrolizumab was discontinued in `r length(unique(aes_discontinuation_pembro$patient))` patients and Lenvatinib was discontinued in `r length(unique(aes_discontinuation_lenva$patient))` patients.

#### **Table 18. Adverse Events (AEs) that led to dose reductions in the ITT population:**

```{r, message = FALSE, warning = FALSE, echo = FALSE}
aes_reductions <- data_14_aes %>%
  filter(str_detect(action_taken, "reduction"))

aes_reductions_pembro <- aes_reductions %>%
  filter(str_detect(action_taken, "Pembrolizumab"))

aes_reductions_lenva <- aes_reductions %>%
  filter(str_detect(action_taken, "Lenvatinib"))

tbl_reductions <- aes_reductions %>%
  tbl_ae(
    id_df = data_02_itt,
    id = patient,
    ae = adverse_event,
...
tbl_reductions %>%
  as_flex_table() %>%
  flextable::save_as_docx(
    path = here(paste0("output/", project_id, "_aes_dose-reductions_table_", as.Date(Sys.Date()), ".docx"))
  )

tbl_reductions
```

There are `r length(unique(aes_reductions$patient))` patients with adverse events that led to dose reductions of any study drug. Pembrolizumab was reduced in `r length(unique(aes_reductions_pembro$patient))` patients and Lenvatinib was reduced in `r length(unique(aes_reductions_lenva$patient))` patients.
