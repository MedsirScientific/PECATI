...
pembrolizumab_modifications <- data_01_cycles %>% 
  ...
  select(patient, event_num, pembrolizumab_administered, reason_no_administration, specify_ae_no_dose, date_of_administration,
...
lenvatinib_modifications_s2 <- read_excel(
  ...
  select(patient, event_num, date, lenvatinib_daily_dose_mg,
...
lenvatinib_modifications <- lenvatinib_modifications_s1 %>% 
  left_join(
    lenvatinib_modifications_s2, by = c("patient", "event_num")
    ) %>% 
  rename(
...
tbl_pembrolizumab_delays <- pembrolizumab_delays %>%
  tbl_summary(
    ...
    sort = list(everything() ~ "frequency"),
    label = list(delay ~ "Pembrolizumab dose delay",
...
tbl_pembrolizumab_reductions <- pembrolizumab_reductions %>%
  tbl_summary(
    ...
    type = all_dichotomous() ~ "categorical",
    sort = list(everything() ~ "frequency"),
    label = list(reduction ~ "Pembrolizumab dose reduction",
...
tbl_pembrolizumab_temporary <- pembrolizumab_temporary_interruptions %>%
  tbl_summary(
    ...
    type = all_dichotomous() ~ "categorical",
    sort = list(everything() ~ "frequency"),
    label = list(temporary_interruption ~ "Pembrolizumab dose temporary interruption",
...
tbl_lenvatinib_temporary <- lenvatinib_temporary_interruptions %>%
  tbl_summary(
    ...
    type = all_dichotomous() ~ "categorical",
    sort = list(everything() ~ "frequency"),
    label = list(temporary_interruption ~ "Lenvatinib temporary dose interruption",
