...
data_00_screening <- read_excel(
    here(paste0("data/PECATI_ICF Screening_", cutoff_date_formatted, ".xlsx")),
    sheet = "ICF Screening",
    skip = 1) %>% 
  clean_names() %>% 
  rename(
...
data_01_cycles <- read_excel(
  here(paste0("data/PECATI_Cycles_", cutoff_date_formatted, ".xlsx")),
    sheet = "Cycles",
    skip = 1) %>% 
  clean_names()
...
data_05_eot <- read_excel(
  here(paste0("data/PECATI_FU30-EOT_", cutoff_date_formatted, ".xlsx")),
    sheet = "FU30-EOT",
    skip = 1
  ) %>% 
  clean_names() %>% 
...
data_06_eos <- read_excel(
  here(paste0("data/PECATI_End of Study_", cutoff_date_formatted, ".xlsx")),
  sheet = "End of Study",
  skip = 1
  ) %>%
  clean_names() %>%
...
data_08b_baseline_nontarget <- read_excel(
    here(paste0("data/PECATI_Tumor Assessment_", cutoff_date_formatted, ".xlsx")),
    sheet = "NON_TARGET",
    skip = 1) %>% 
    clean_names() %>% 
    # The variable "event_num" encodes the tumour assessment number in the post-baseline assessments.
