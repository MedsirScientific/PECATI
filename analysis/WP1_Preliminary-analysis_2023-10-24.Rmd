...
data_00_screening <- read_excel(
    here(paste0("data/", project_id, "_ICF Screening_", cutoff_date_formatted, ".xlsx")),
    sheet = "ICF Screening",
    skip = 1) %>% 
  clean_names()
...
data_01_cycles <- read_excel(
  here(paste0("data/", project_id, "_Cycles_", cutoff_date_formatted, ".xlsx")),
    sheet = "Cycles",
    skip = 1) %>% 
  clean_names()
...
data_05_eot <- read_excel(
  here(paste0("data/", project_id, "_FU30-EOT_", cutoff_date_formatted, ".xlsx")),
    sheet = "FU30-EOT",
    skip = 1
  ) %>% 
  clean_names() %>% 
...
data_06_eos <- read_excel(
  here(paste0("data/", project_id, "_End of Study_", cutoff_date_formatted, ".xlsx")),
  sheet = "End of Study",
  skip = 1
  ) %>%
  clean_names() %>%
...
data_08b_baseline_nontarget <- read_excel(
    here(paste0("data/", project_id, "_Tumor Assessment_", cutoff_date_formatted, ".xlsx")),
    sheet = "NON_TARGET",
    skip = 1) %>% 
    clean_names() %>% 
    # The variable "event_num" encodes the tumour assessment number in the post-baseline assessments.
...
swimmer_spider_plot <- function(site, comment, name) {
  ...
  ggsave(
    paste("../output/swimmer-spider-plot/materials/", project_id, "_site-", as.character({{site}}), "_swimmer_TAs_", cutoff_date, ".png", sep = ""),
    swimmer_ta,
    width = 20,
    height = 20,
    units = "cm",
  ...
  ggsave(
    paste("../output/swimmer-spider-plot/materials/", project_id, "_site-", as.character({{site}}), "_spider_TAs_", cutoff_date, ".png", sep = ""),
    spider_ta,
    width = 15,
    height = 20,
    units = "cm",
  ...
  spider_ta <- spider_ta +
    annotate("rect", xmin = 0, xmax = 8, ymin = -30, ymax = -100,
             alpha = .1,fill = "#74AAFF") +
    annotate("rect", xmin = 0, xmax = 8, ymin = 20, ymax = 100,
             alpha = .0, fill = "#B24745")
  ...
  ggsave(
    paste("../output/swimmer-spider-plot/", project_id, "_site-", as.character({{site}}), "_TAs_", cutoff_date, ".png", sep = ""),
    combined_ta,
    width = 30,
    height = 75,
    units = "cm",
...
Patient 0102-001 discontinued study treatment after 19 cycles due to unacceptable toxicity (grade 3 pneumonitis related to Pembrolizumab).
...
Patient 0102-004 discontinued study treatment after 1 cycle without any post-baseline tumor assessment due to a second primary esophageal cancer.
...
Patient 0103-001 discontinued study treatment after 14 cycles due ton radiological progression (unequivocal progression of non-target lesions).

Patient 0103-002 is stable after 20 cycles of study treatment (last registered dose: 2023-10-11).
...
Patient 0105-001 is stable after 23 cycles of study treatment (last registered dose: 2023-08-30).

Patient 0105-003 is stable after 16 cycles of study treatment (last registered dose: 2023-09-05)
...
Patient 0201-001 is responding after 11 cycles of study treatment (last registered dose: 2023-10-04).

Patient 0201-002 is stable after 10 cycles of study treatment (last registered dose: 2023-09-11).

Patient 0201-003 is stable after 10 cycles of study treatment (last registered dose: 2023-10-09).

Patient 0201-004 is stable after 7 cycles of study treatment (last registered dose: 2023-10-11).

Patient 0201-005 is receiving the study treatment after 2 cycles but does not have any post-baseline tumor assessment yet (last registered dose: 2023-07-27).
...
Patient 0202-002 discontinued the study treatment after 3 cycles due to unacceptable toxicity (unrelated grade 4 severe sepsis on pancolitis).

Patient 0202-003 is stable after 11 cycles of study treatment (last registered dose: 2023-02-22).

Patient 0202-004 is stable after 12 cycles of study treatment (last registered dose: 2023-09-06).

Patient 0202-005 is receiving the study treatment after 10 cycles but does not have any tumor assessment registered in the eCRF (last registered dose: 2023-08-04).

Patient 0202-007 is receiving the study treatment after 7 cycles but does not have any tumor assessment registered in the eCRF (last registered dose: 2023-08-09).
...
Patient 0203-001 is receiving the study treatment after 6 cycles but does not have any tumor assessment registered in the eCRF (last registered dose: 2023-07-31).

Patient 0203-002 is receiving the study treatment after 2 cycles but does not have any post-baseline tumor assessment yet (last registered dose: 2023-09-18).
...
Patient 0204-001 is stable after 6 cycles of study treatment (last registered dose: 2023-09-27).
...
Patient 0205-001 is responding after 7 cycles of study treatment (last registered dose: 2023-09-20).
...
Patient 0206-001 is responding after 5 cycles of study treatment (last registered dose: 2023-10-04).

Patient 0206-002 just started the study treatment on 2023-10-04.

```{r site-0301-swimmer-spider-plot, message = FALSE, warning = FALSE, echo = FALSE}
...
Patient 0301-001 discontinued the study treatment after 11 cycles due to general or specific changes in the patient's condition render the patient unacceptable for further treatment.

Patient 0301-002 is stable after 21 cycles of study treatment (last registered dose: 2023-10-05).

Patient 0301-003 is responding after 18 cycles of study treatment (last registered dose: 2023-10-12).

Patient 0301-004 is stable after 14 cycles of study treatment (last registered dose: 2023-09-27).

Patient 0301-005 is stable after 10 cycles of study treatment (last registered dose: 2023-09-27).

Patient 0301-006 is stable after 10 cycles of study treatment (last registered dose: 2023-10-05).

Patient 0301-007 is stable after 9 cycles of study treatment (last registered dose: 2023-10-11).

Patient 0301-008 is responding after 2 cycles of study treatment (last registered dose: 2023-08-03).

Patient 0301-009 is stable after 5 cycles of study treatment (last registered dose: 2023-10-10).

Patient 0301-010 is stable after 5 cycles of study treatment (last registered dose: 2023-10-13).

Patient 0301-011 is stable after 4 cycles of study treatment (last registered dose: 2023-10-06).
...
#### **Figure 2. 5-month Progression-free survival (PFS) in the intention-to-treat population**
...
caption1 <- paste(strwrap(
  "5-month PFS is 97.0% (95% CI 80.4-99.6)", 90), collapse = "\n"
  )

pfs_5mo$plot <- pfs_5mo$plot + annotate(
  "text", x = 0.5, y = 0.10,
  label = caption1,
  cex = 2.5, vjust = "center", hjust = "left", fontface = 20
  )

### Save plot ---------------------------------------------------------------
ggsave(
  here("output/", project_id, "_Figure_5-mo_PFS_global_", as.Date(Sys.Date()), ".png"),
  pfs_5mo,
  width = 20,
  height = 12,
  units = "cm",
...
#### **Figure 3. Overall survival (OS) in the intention-to-treat popilation**
...
caption1 <- paste(strwrap(
  "Medidan OS is Not Reached (NR - NR). Events: 1/34 (2.9%)", 90), collapse = "\n"
  )

caption2 <- paste(strwrap(
  "12-month OS is 97% (95% CI 91.3-100)", 90), collapse = "\n"
  )
...
### Save plot ---------------------------------------------------------------
ggsave(
  here(paste0("output/", project_id, "_Figure_OS_global_", as.Date(Sys.Date()),".png")),
  os_global,
  width = 20,
  height = 12,
  units = "cm",
...
caption1 <- paste(strwrap(
  "Medidan PFS is Not Reached (NR - NR). Events: 1/34 (2.9%)", 90), collapse = "\n"
  )

caption2 <- paste(strwrap(
  "12-month PFS is 90.9% (95% CI 75.4-100)", 90), collapse = "\n"
  )
...
### Save plot ---------------------------------------------------------------
ggsave(
  here("output/", project_id, "_Figure_PFS_global_", as.Date(Sys.Date()), ".png"),
  pfs_global,
  width = 20,
  height = 12,
  units = "cm",
...
data_14_aes <- read_excel(
    here(paste0("data/", project_id, "_Adverse Events_", cutoff_date_formatted, ".xlsx")),
...
#### **Table 9. Adverse Events (AEs) by maximum severity in the ITT population:**
...
tbl_aes %>%
  ...
  flextable::save_as_docx(
    path = here(paste0("output/", project_id, "_aes_table_", cutoff_date_formatted, ".docx"))
  )

tbl_aes
```

#### **Table 10. Serious Adverse Events (SAEs) by maximum severity in the ITT population:**
...
tbl_saes %>%
  as_flex_table() %>%
  flextable::save_as_docx(
    path = here(paste0("output/", project_id, "_saes_table_", cutoff_date_formatted, ".docx"))
  )

tbl_saes
```

#### **Table 11. Adverse Events (AEs) that led to the permanent discontinuation of the study drug in the ITT population:**
...
tbl_discontinuations %>%
  as_flex_table() %>%
  flextable::save_as_docx(
    path = here(paste0("output/", project_id, "_aes_discontinuations_table_", cutoff_date_formatted, ".docx"))
  )
